...
export async function getServerSideProps (pageContext) {
    ...
    let apiResponse = await fetch(
        `https://api.stackexchange.com/2.2/search?fromdate=${weekBefore}&todate=${currTime}&order=desc&sort=votes&tagged=${tagged}&filter=!)rTkraPXxg*xgr03n8Uq&site=stackoverflow&key=${process.env.STACK_EXCHANGE_KEY}`
    );
    ...
    apiResponse = await fetch(
        `https://api.stackexchange.com/2.2/search?fromdate=${weekBefore}&todate=${currTime}&order=desc&sort=creation&tagged=${tagged}&filter=!)rTkraPXxg*xgr03n8Uq&site=stackoverflow&key=${process.env.STACK_EXCHANGE_KEY}`
    );
